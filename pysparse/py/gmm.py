...
# from utils import initialize_flat_gmm, initialize_gmm_components_with_noise

class GMMHead(nn.Module):
    def __init__(self, d_input: int, d_hidden: int, num_hidden: int, num_components: int, activation: str):
        super().__init__()

        # Policy networks for means, stds, and weights
        self.mean_net = LinHead("actor", d_input, d_hidden, num_hidden, num_components * 5, activation)
        ...
        self.mean_std = nn.Parameter(torch.zeros(1, num_components, 2) - 1)
        self.logstd_std = nn.Parameter(torch.zeros(1, num_components, 2) - 1)
        self.weights_std = nn.Parameter(torch.zeros(1, num_components) - 1)
    ...
    def initialize_parameters(self):
        ...
        init_means, init_logstds, init_weight_logits = initialize_gmm_components_with_noise(num_components=self.num_components, noise_scale=0.3, std_size=1.5)
