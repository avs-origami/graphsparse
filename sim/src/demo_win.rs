...
impl Win {
    ...
    pub fn update(&mut self, kmask: &Vec<u32>) -> Result<()> {
        ...
        let out: Vec<u32> = self.buf.iter().zip(kmask).map(|(n, m)| {
            n * m
        }).collect();

        let out2: Vec<u32> = out.iter().zip(&self.toplayer).map(|(n, m)| {
            if *m != 0 {
                *m
            } else {
                *n
        ...
        if let Some(ref mut x) = self.win { x.update_with_buffer(&out2, WIDTH, HEIGHT)?; }
    ...
    pub fn overlay(&mut self, pix: Vec<u32>) -> Result<()> {
        self.toplayer = pix;
        Ok(())
    }

    pub fn erase_overlay(&mut self) -> Result<()> {
