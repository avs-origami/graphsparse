...
use rrt_demo::{Exit, RrtInc};
use sim::demo_api::{Cmd::*, Req::*, SimApi};
...
fn collect_eval(rithm: &mut RrtInc, py: &mut Bridge, rewards: &mut Vec<f32>, covs: &mut Vec<f32>, sim_thread: &mut JoinHandle<()>) -> Result<()> {
    ...
    'o: for i in 0..rithm.opts.num_moves {
        ...
        py.next_eval(i, reward, terminal)?;
...
fn export_state(rithm: &mut RrtInc, last_buf: &mut Vec<u32>, count: usize) -> usize {
    ...
    let mut tree = vec![0; WIDTH * HEIGHT];
    ...
    for (_, node) in &rithm.rrt.tree {
        ...
        if let Some(x) = node.par.lock().unwrap().upgrade() {
            ...
            for p in l {
                if p < buf.len() { if buf[p] != col::RED { buf[p] = col::EDGE; tree[p] = col::EDGE; }}
            }

            for p in c {
                if p < buf.len() { if buf[p] != col::RED { buf[p] = col::NODE; tree[p] = col::NODE; }}
            }
        }
    }

    rithm.sml.cmd(DrawTree(tree)).unwrap();
